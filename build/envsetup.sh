...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/tenx-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.tenx.device | grep -q "$TENX_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $TENX_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.tenx.device | grep -q "$TENX_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.tenx.device | grep -q "$TENX_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/tenx/build/tools/repopick.py $@
}
