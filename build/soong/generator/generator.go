...
func init() {
	android.RegisterModuleType("tenx_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = tenxExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = tenxExpandVariables(ctx, dep_file)
	...
	cmd := tenxExpandVariables(ctx, String(g.properties.Cmd))
